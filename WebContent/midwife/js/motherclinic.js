	...
	xmlhttp.onreadystatechange = function() {
		if(xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			if(out.count!='undefined' && out.count!='0')
				document.getElementById("count").innerHTML = "<center><h2>"+out.count+" mothers didn't attend for the clinic"+"</h2></center>";
			else
				document.getElementById("count").innerHTML = "";
			var k = Object.keys(out);
			for(var i=0;i<k.length;i++){
				if((k[i]!='venue')&&(k[i]!='time')&&(k[i]!='count'))
					addChildren(out[k[i]].name, out[k[i]].age, out[k[i]].vname, out[k[i]].vamount,"pastMotherTable");
			}
			k = Object.keys(out2);
			for(var i=0;i<k.length;i++){
