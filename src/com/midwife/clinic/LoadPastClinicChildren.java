...
@WebServlet("/loadpastclinicchildren")
public class LoadPastClinicChildren extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		PrintWriter out = response.getWriter();
		String area = request.getParameter("area");
		String date = request.getParameter("date");
		ClinicUnvisited clinic = new ClinicUnvisited(area, date);
		ArrayList<ChildInClinic> mothers = clinic.getUnvisitedChildren();
		int unvisitedcount = clinic.getUnvisitedChildrenCount();
